...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # or alternatively, if you have a custom self-attention implementation:
...
class DataLoaderLite:
    def __init__(self,B,T):
        ...
        with open('../BigramGPT/input.txt', 'r') as f:
            text = f.read()
    ...
    def next_batch(self):
        ...
        # if loading batch is out of bounds then reset
...
train_loader = DataLoaderLite(B = 64, T = 32)
...
while x.size(1) < max_length:
    logits, _ = model(x)
    # take the logits at the last position and divide by a temperature factor
    logits = logits[:, -1, :]
    # get the probabilities 
    probs = F.softmax(logits, dim=-1)
    # do top k sampling of 50 (hugging face pipeline default)
    # topk probs here becomes (5,50), topk indices is (5,50)
...
for i in range(num_return_sequences):
    # convert the generated indices to tokens
